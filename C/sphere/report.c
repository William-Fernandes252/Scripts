...
void print_report(const unsigned long n, const unsigned long r, const result *const results) {
	...
	for(int l = 0; l != LAYERS; l++)
	    printf("%-11d%13.5e %c %-13.5e\n", (l+1), results->n_coll_mean[l], (char)241, results->n_coll_dev[l]);
...
void write_report(const unsigned long n, const unsigned long r, const result *const results) {
	...
	fprintf(report, "%-9s%-s\n", "Layer", "Collisions");	
	for(int l = 0; l != LAYERS; l++)
	    fprintf(report, "%-8d%13.5e +/- %-13.5e\n", (l+1), results->n_coll_mean[l],results->n_coll_dev[l]);
...
void csv_report(const unsigned long n, const unsigned long r, const result *const results) {
	...
	if(option == 's') {
		...
		puts("Write average number of collisions per layer (c) or total flux vs. radius (f)?");
		do {
		option = tolower(getchar());
		} while (option != 'c' && option != 'f');

		write_data(report, option, sep, true, results);
		...
		write_data(report, option, sep, false, results);
...
void write_data(FILE *const report, char option, char sep, bool new_report, const result *const results) {
	...
	switch(option) {
		...
		case 'c':
			...
			for(int l = 0; l != LAYERS; l++)
    			fprintf(report, "%d%c%.5e%c%.5e\n", (l+1), sep, results->n_coll_mean[l], sep, results->n_coll_dev[l]);
